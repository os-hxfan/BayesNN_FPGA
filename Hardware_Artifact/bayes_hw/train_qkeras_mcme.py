...
import argparse 
import numpy as np
from models import lenet, ResNet20ForCIFAR10, VGG11
from qmodels import QResNet18, QVGG11
from t_qmodels_bayes import T_Qlenet_bayes, T_QVGG11_bayes, T_QResNet18_bayes
from s_qmodels_bayes import S_Qlenet_bayes, S_QVGG11_bayes, S_QResNet18_bayes
from t_qmodels_bayes_me import T_Qlenet_bayes_me, T_QResNet18_bayes_me, T_QVGG11_bayes_me
...
def get_model(args):
    ...
    if args.model_name == "lenet":
        if args.opt_mode == "temporal":
            model, mc_model = T_Qlenet_bayes(args, model_num_layer[args.model_name])
            if args.is_me != 0:
                model, mc_model = T_Qlenet_bayes_me(args, model_num_layer[args.model_name])
        else:
            model, mc_model = S_Qlenet_bayes(args, model_num_layer[args.model_name])
    elif args.model_name == "resnet":
        if args.opt_mode == "temporal":
            model, mc_model = T_QResNet18_bayes(input_shape=(32, 32, 3), classes=10, args=args, weight_decay=1e-4, base_filters=16)
            if args.is_me != 0:
                model, mc_model = T_QResNet18_bayes_me(input_shape=(32, 32, 3), classes=10, args=args, weight_decay=1e-4, base_filters=16)
        else:
            model, mc_model = S_QResNet18_bayes(input_shape=(32, 32, 3), classes=10, args=args, weight_decay=1e-4, base_filters=16)
    elif args.model_name == "vgg":
        if args.opt_mode == "temporal":
            model, mc_model = T_QVGG11_bayes(args, filters=16, dense_out=[16, 16, 10])
            if args.is_me != 0:
                model, mc_model = T_QVGG11_bayes_me(args, filters=16, dense_out=[16, 16, 10])
        else:
            model, mc_model = S_QVGG11_bayes(args, filters=16, dense_out=[16, 16, 10])
    else:
...
def train(args, model, dataset): 
    ...
    if args.model_name == "lenet":
        if args.is_me == 0:
            dataset_y = dataset['y_train']
        else:
            # We only define two exits for each model.  
            # If more exits are requires, pls also change the architecture defination in the corresponding file and args.num_exits
            dataset_y = [dataset['y_train'] for i in range(args.num_exits)] 
        train_stat = model.fit(
            dataset['x_train'], dataset_y, batch_size=args.batch_size,
            epochs=args.num_epoch, initial_epoch=1, callbacks=[CosineAnnealingScheduler(T_max=args.num_epoch, eta_max=args.lr, eta_min=1e-4)],
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_dir", default=None, type=str, help="Directory name of saved results")
    ...
    parser.add_argument("--opt_mode", default="spatial", type=str, help="Optimization mode for MC sampling part", choices=["spatial", "temporal"])
    ...
    parser.add_argument("--scale", default=4.0, type=float, help="The scale")
    ...
    if args.load_model is None:
        model, mc_model = get_model(args)     
        mc_model = Top_Level_Model(args, mc_model)       
    else:
    ...
    if (args.opt_mode == "spatial") and args.is_train:
        weight = (model.layers)[0].get_layer('fc_final').get_weights()
        for i in range(args.mc_samples-1):
            layer_name = 'mc_fc_final_%i'%(i)
            (mc_model.layers)[0].get_layer(layer_name).set_weights(weight)

    if args.save_model is not None: 
        mc_model.model.save(args.save_model+'.h5')
